 ...
 * @since      1.0.0
 */
class blackvideo_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blackvideo-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'blackvideo' )
		);

		// Create the widget.
		parent::__construct(
			'blackvideo-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'blackvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'blackvideo_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'blackvideo_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'blackvideo_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'blackvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
