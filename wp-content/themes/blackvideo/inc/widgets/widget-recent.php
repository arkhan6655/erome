 ...
 * @since      1.0.0
 */
class blackvideo_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blackvideo-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'blackvideo' )
		);

		// Create the widget.
		parent::__construct(
			'blackvideo-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'blackvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'blackvideo_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'blackvideo_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'blackvideo_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'blackvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
