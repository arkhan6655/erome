 ...
 * @since      1.0.0
 */
class BlackVideo_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blackvideo-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'blackvideo' )
		);

		// Create the widget.
		parent::__construct(
			'blackvideo-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'blackvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'blackvideo_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'blackvideo_most_commented_widget_' . $this->id, $popular );
					...
					echo '<li class="clear">';
						if ( has_post_thumbnail() ) {
							...
							echo '<a class="thumbnail-link" href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
								...
								if( (blackvideo_has_embed_code() || blackvideo_has_embed()) ) { 
						...
						if ( $instance['show_date'] ) :
							echo '<div class="entry-meta">' . esc_html( human_time_diff(get_the_time('U'), current_time('timestamp')) ) . ' '.  esc_html( 'ago', 'blackvideo' ) . '</div>';
						endif;
					echo '</div></li>';
	...
	function flush_widget_transient() {
		delete_transient( 'blackvideo_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'blackvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_date' ) ); ?>">
				<?php esc_html_e( 'Display post date?', 'blackvideo' ); ?>
			</label>
		</p>
