 ...
 * @since      1.0.0
 */
class BlackVideo_Home_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blackvideo-home-block-one',
			'description' => __( 'Display post content blocks. Only use for the "Home Content" widget area.', 'blackvideo' )
		);

		// Create the widget.
		parent::__construct(
			'blackvideo-home-block-one',          // $this->id_base
			__( '&raquo; Home Content', 'blackvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			$query = apply_filters( 'blackvideo_home_one_column_args', $args );
					...
					<?php
						...
						if ( $cat_id == 0 ) {
							if ( empty( $instance['title'] ) ) {
								echo '<h3 class="section-title"><span>' . wp_kses_post( apply_filters( 'widget_title',  esc_html( 'Latest Posts', 'blackvideo' ), $instance, $this->id_base ) ) . '</span></h3>';
							} else {
							...
							echo '<span class="section-more-link"><a href="' . esc_url( $cat_link ) . '">'. esc_html( 'More', 'blackvideo' ) . '</a></span>';			
						}
					...
					<div <?php post_class( $grid_col ); ?>>
						...
						<div class="entry-header">
							...
							<div class="entry-meta">
								<span class="entry-date"><?php echo esc_html( human_time_diff(get_the_time('U'), current_time('timestamp')) ) . ' '.  esc_html( 'ago', 'blackvideo' ); ?></span>
							</div><!-- .entry-meta -->		
						</div><!-- .entry-header -->
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title:', 'blackvideo' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Choose category', 'blackvideo' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'cat' ) ); ?>" style="width:100%;">
				...
				<option value="0"><?php esc_html_e( 'All categories &hellip;', 'blackvideo' ); ?></option>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
				...
				<?php esc_html_e( '3 Columns Layout', 'blackvideo' ); ?>		    	
			</label>		    
		<br/>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	...
		    	<?php esc_html_e( '4 Columns Layout', 'blackvideo' ); ?>
			</label>		    
	    </p>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'blackvideo' ); ?>
			</label>
