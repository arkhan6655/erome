 ...
 * @since      1.0.0
 */
class VideoShare_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videoshare-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'videoshare' )
		);

		// Create the widget.
		parent::__construct(
			'videoshare-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'videoshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'videoshare_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'videoshare_most_commented_widget_' . $this->id, $popular );
					...
					echo '<li class="clear">';
						if ( has_post_thumbnail() ) {
							...
							echo '<a class="thumbnail-link" href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
								...
								if( (videoshare_has_embed_code() || videoshare_has_embed()) ) { 
						...
						if ( function_exists( 'pvc_get_post_views' ) ) {
							...
							echo '<span class="entry-views">';
								echo videoshare_custom_number_format($views) . ' ' . esc_html('views', 'videoshare');
							echo '</span>';

						}

						if ( function_exists( 'pvc_get_post_views' ) && ($instance['show_date']) ) {
							echo '<span class="sep"> &middot; </span>';
						}
						
						if ( $instance['show_date'] ) :
							echo esc_html( human_time_diff(get_the_time('U'), current_time('timestamp')) ) . ' '.  esc_html( 'ago', 'videoshare' );
						endif;
	...
	function flush_widget_transient() {
		delete_transient( 'videoshare_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'videoshare' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_date' ) ); ?>">
				<?php esc_html_e( 'Display post date?', 'videoshare' ); ?>
			</label>
		</p>
