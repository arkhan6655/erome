 ...
 * @since      1.0.0
 */
class VideoShare_Home_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videoshare-home-block-one',
			'description' => __( 'Display post content blocks. Only use for the "Home Content" widget area.', 'videoshare' )
		);

		// Create the widget.
		parent::__construct(
			'videoshare-home-block-one',          // $this->id_base
			__( '&raquo; Home Content', 'videoshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			$query = apply_filters( 'videoshare_home_one_column_args', $args );
			...
			$col = isset( $instance['col'] ) ? strip_tags( $instance['col'] ) : '5col';

			$grid_col = 'ht_grid_1_5';
				...
				<div class="content-block content-block-1 clear">
					...
					<?php
						if ( ( ! empty( $instance['title'] ) ) && ( $cat_id != 0 ) ) {
							...
							echo '<span class="section-more-link"><a href="' . esc_url( $cat_link ) . '">'. esc_html( 'More', 'videoshare' ) . '</a></span>';

						} elseif ( $cat_id == 0 ) {

							echo '<h3 class="section-title"><span>' . wp_kses_post( apply_filters( 'widget_title',  esc_html( 'Latest Posts', 'videoshare' ), $instance, $this->id_base ) ) . '</span></h3>';
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title:', 'videoshare' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Choose category', 'videoshare' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'cat' ) ); ?>" style="width:100%;">
				...
				<option value="0"><?php esc_html_e( 'All categories &hellip;', 'videoshare' ); ?></option>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	<input class="" id="<?php echo esc_html( $this->get_field_id('5col') ); ?>" name="<?php echo esc_html( $this->get_field_name('col') ); ?>" type="radio" value="5col" <?php if($col === '5col') { echo 'checked="checked"'; } ?> />
				<?php esc_html_e( '5 Columns Layout', 'videoshare' ); ?>		    	
			</label>		    
		<br/>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	...
		    	<?php esc_html_e( '4 Columns Layout', 'videoshare' ); ?>
			</label>		    
	    </p>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'videoshare' ); ?>
			</label>
