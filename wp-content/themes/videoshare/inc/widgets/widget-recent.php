 ...
 * @since      1.0.0
 */
class videoshare_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videoshare-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'videoshare' )
		);

		// Create the widget.
		parent::__construct(
			'videoshare-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'videoshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'videoshare_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'videoshare_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'videoshare_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'videoshare' ),
			'limit'     => 5,
			'show_date' => true
		);
