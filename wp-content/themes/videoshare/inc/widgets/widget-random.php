 ...
 * @since      1.0.0
 */
class videoshare_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videoshare-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'videoshare' )
		);

		// Create the widget.
		parent::__construct(
			'videoshare-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'videoshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'videoshare_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'videoshare_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'videoshare_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'videoshare' ),
			'limit'     => 5,
			'show_date' => true
		);
